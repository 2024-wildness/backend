...
public class PhotoController {
  ...
  public ResponseEntity<PhotoUploadUrlResponse> getUploadUrl(
    ...
    ContentService.PhotoUploadInfo uploadInfo =
        contentService.createPhotoUploadUrl(request.getFileName(), request.getContentType());
  ...
  public ResponseEntity<ContentResponse> completeUpload(
    ...
    Content content =
        contentService.completePhotoUpload(
            request.getCategoryId(), request.getTitle(), request.getKey());
    return ResponseEntity.ok(ContentResponse.fromEntity(content));
  }

  @GetMapping("/{id}/download-url")
  public ResponseEntity<PhotoDownloadUrlResponse> getDownloadUrl(@PathVariable Long id) {
